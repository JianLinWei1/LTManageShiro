...
public class LoginController {
	...
	public String syslogin(HttpServletRequest request, Model model) {
		...
		if (StringUtils.isEmpty(sysAdmin) || StringUtils.isEmpty(password)) {
			model.addAttribute("msg", "参数不能为空");
			return "login";
		}
		try {
			Subject  subject   = SecurityUtils.getSubject();
			UsernamePasswordToken usernamePasswordToken   = new UsernamePasswordToken(sysAdmin, password);
		 ...
		 UserRole  uro =	ur.selectUser(sysAdmin);
		request.getSession().setAttribute("user", sysAdmin);
		request.getSession().setAttribute("UserRole", uro);
		model.addAttribute("user", uro);
		List<UserRole> lu = ur.selectMenus(uro.getRoleId(), uro.getUsername());
		for (int i = 0; i < lu.size(); i++) {
			model.addAttribute("lu" + lu.get(i).getMenusId(), lu.get(i).getMenusId());
		}
	...
	public void grpcmainserver() {
		...
		/*
		 * ReadICcardThread ric = new ReadICcardThread(as, es); Thread t = new
		 * Thread(ric); t.setPriority(Thread.MAX_PRIORITY); if (redcard == 1) {
		 * t.start(); }
		 */
	}
		...
		for (int i = 1; i < page; i++) {
			...
			for (int j = 0; j < la.size(); j++) {
				...
				if (x < 0) {
					...
					logger.info("清理》》》工号：" + la.get(j).getEmployeeId() + "姓名：" + la.get(j).getName() + "打卡时间："
							+ sdf.format(la.get(j).getWorkDate()));
				}
			}
