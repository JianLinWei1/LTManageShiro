...
import com.jian.ssm.util.CutImageUtil;
...
public class PeopleIdentityImpl  implements  PeopleIdentityService {
	...
	public List<PeopleIdentity> selectPeopleIdentityByBelongId(int befor, int after, UserRole ur) {
		CutImageUtil   cu  = new CutImageUtil();
		Base64.Encoder  en  =Base64.getEncoder();
		List<PeopleIdentity>  lp  = pid.selectPeopleIdentityByBelongId(befor, after, ur.getBelongid());
		for(PeopleIdentity  p : lp){
			/*p.setIdphoto(cu.compressImg(p.getIdphoto()));*/
			p.setCameraphoto(cu.compressImg(p.getCameraphoto()));
			p.setIdphoto_s("<img  id="+UUID.randomUUID()+"  onclick=\"takeImg(this)\" height=\"60px\" src=\" " + "data:image/jpeg;base64,"
					+ en.encodeToString(p.getIdphoto())+ "\">");
			p.setIdphoto(null);
			p.setCameraphoto_s("<img id="+UUID.randomUUID()+"  onclick=\"takeImg(this)\"  onclick=\"takeImg(this)\" height=\"60px\" src=\" " + "data:image/jpeg;base64,"
	...
	public List<PeopleIdentity> selectPeopleIdentityByName(int befor, int after, UserRole ur, String name) {
		...
		for(PeopleIdentity  p : lp){
			...
			p.setIdphoto_s("<img  id="+UUID.randomUUID()+"  onclick=\"takeImg(this)\" height=\"60px\" src=\" " + "data:image/jpeg;base64,"
					+ en.encodeToString(p.getIdphoto())+ "\">");
			p.setIdphoto(null);
			p.setCameraphoto_s("<img  id="+UUID.randomUUID()+"  onclick=\"takeImg(this)\" height=\"60px\" src=\" " + "data:image/jpeg;base64,"
							+ en.encodeToString(p.getCameraphoto()) + "\">");
			p.setCameraphoto(null);
			if(!ur.getUsername().equals("admin")){
				String idcard = p.getIdcard();
